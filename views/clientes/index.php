...
<?php echo Dialog::widget(); ?>
  ...
  <?= GridView::widget([
    ...
    'panel' => [
          'heading'=>'<h3 class="panel-title"><i class="fa fa-exclamation"></i> Clientes</h3>',
          'type'=>'info',
        ...
        'columns' => [
                  ...
                  'buttons' => [
                    'update' => function ($url, $model, $key) {
                        return Html::a('<span class="btn btn-xs btn-primary"><i class="ace-icon fa fa-refresh bigger-120"></i></span> ',
                            Url::to(['clientes/update','id'=>$model->id]), [
                ...
                [
                      ...
                      'buttons' => [
                        'delete' => function ($url,$model, $key) {
                              $url=Url::to(['clientes/delete','id'=>$model->id]);
                              return Html::a('<span class="btn btn-xs btn-danger"><i class="ace-icon fa fa-trash bigger-120"></i></span> ', '#', [
                                  'title' => Yii::t('yii', 'Delete'),
                                  'aria-label' => Yii::t('yii', 'Delete'),
                                  'onclick' => "
                                  krajeeDialog.confirm('Esta seguro de eliminar el Tipo de Documento:  ' +  '$model->cedrif', function (result) {
                                       if (result) {
                                          $.ajax({
            ...
            [
              ...
              'value'=>function($model){
                return isset($model->zona) ? $model->zona->denominacion : 'No Asignado';
              },
              'filter' => Html::activeDropDownList($searchModel,
              'id_zona', ArrayHelper::map(app\models\Zonas::find()->all(),
